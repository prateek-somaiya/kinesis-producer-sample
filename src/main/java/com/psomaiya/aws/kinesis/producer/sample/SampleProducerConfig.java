...
//	@Min(value = 2, message = "KPL Sample aggregationMaxSize should not be less than 2")
	 ...
	 ** 
	 * @param args The command line args for the Sample Producer. It takes 13
	 *             optional position parameters: 1. The stream name to use, test is
	 *             default. 2. The region name to use, us-west-1 in default. 3. The
	 *             duration of the test in seconds, 5 is the default. 4. The number
	 *             of records per second to send, 2000 is the default. 5. The
	 *             payload size of each record being sent in bytes, 128 is the
	 *             default. 6. The max number of connections to configure the KPL
	 *             with, 1 is the default. 7. The requestTimeout in milliseconds to
	 *             configure the KPL with, 60000 is the default. 8. The bufferTime
	 *             in milliseconds to configure the KPL with, 2000 is the default.
	 *             9. The threading model to configure the KPL with, PER_REQUEST is
	 *             the default. 10. The threadPoolSize to configure the KPL with, 0
	 *             is the default. 11. The aggregationEnabled to configure the KPL
	 *             with, true is the default. 12. The aggregationMaxCount to
	 *             configure the KPL with, 4294967295 is the default. 13. The
	 *             aggregationMaxSize to configure the KPL with, 51200 is the
	 *             default.
	 */
	public SampleProducerConfig(String[] args) {
		...
		threadingModel = getArgIfPresent(args, argIndex++,
				KinesisProducerConfiguration.ThreadingModel.PER_REQUEST.name());
